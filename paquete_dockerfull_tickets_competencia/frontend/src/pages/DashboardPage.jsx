...
const isPrivileged = (role) => ['ADMIN', 'SUPERVISOR'].includes(role)
...
export default function DashboardPage() {
  ...
  const privileged = isPrivileged(currentUser?.roleCode)

  useEffect(() => {
    const promises = [getCoverageApi().then((r) => setData(r.data?.data)).catch(() => {})]
    if (privileged) {
      promises.push(listBatchesApi().then((r) => setBatches((r.data?.data || []).slice(0, 5))).catch(() => {}))
    }
    Promise.all(promises).catch(() => setError('Error al cargar datos'))
  }, [privileged])
